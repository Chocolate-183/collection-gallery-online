...
- **精細篩選 Modal (`Filter`)**: 展廳篩選列只保留 `ALL` / `RANDOM 10` / `LATEST 10`。子音、字數、種類（外來語）與排序（正序／倒序 × ID／Title／Reading／Gloss）改由 `Filter` 按鈕開啟的設定 Modal 處理。C103 顯示 Starts with / Length / Category / Title / Gloss；C101 顯示 Starts with（あ行…わ行）/ Length / Reading。
